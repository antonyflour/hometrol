...
def http_get(ip, port, request):
    try:
        httpServ = httplib.HTTPConnection(ip, int(port))
        httpServ.connect()
        ...
        if mac in schede:
            ...
            if pin != None and pin.tipo == "O": #posso settare solo se il pin e' di output
                ...
                if stato == 0 or stato == 1:
                    status, response = http_get(scheda.ip, scheda.port, URIPath.URI_SET_STATUS+"?PIN"+str(pin.numero)+"="+str(stato))
                    if response != "":
                        response = formatResponse(200, "Comando eseguito")
                    else:
                        response = formatResponse(commons.ErrorCode.ERROR_SHIELD_COMMUNICATION_NUMBER,
...
def aggiorna_stato_schede():
    for shield in schede.itervalues():
        status, response = http_get(shield.ip, shield.port, URIPath.URI_INPUT_STATUS)
        if response is not "":
            ...
            status, response = http_get(shield.ip, shield.port, URIPath.URI_OUTPUT_STATUS)
            if response is not "":
